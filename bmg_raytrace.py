...
def save_as(data):
	...
	datestring = '{0:d}-{1:02d}-{2:02d}_{3:02d}-{4:02d}-{5:02d}'.format(p.tm_year, p.tm_mon, p.tm_mday, p.tm_hour, p.tm_min, p.tm_sec)
...
if __name__ == '__main__':
	...
	for j in range(det.ydim):
		...
		t2 = time.time()
		print '{0:04d}.\t{1:.2f}% done in {2:.2f} min, (total {3:.2f} min), remaining {4:.2f} min\n'.format((j+1), 1e2*(j+1)/det.ydim, (t2 - t0)/60.0, det.ydim*(t2 - t0)/(60.0*(j+1)), det.ydim*(t2 - t0)/(60.0*(j+1)) - (t2 - t0)/60.0)
